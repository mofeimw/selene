...
(defun helios/buffer/spawn (buffer-name content-function &optional args)
  ...
  (with-current-buffer (get-buffer-create (format "*helios/%s*" buffer-name))
    ...
    (let ((inhibit-read-only t))
      ...
      (if (string= buffer-name "main")
          ;; enable jumping forward/backward between days with arrow keys in main thread
          (progn
            (evil-local-set-key 'normal (kbd "<")
                                `(lambda ()
                                   (interactive)
          ...
          (evil-local-set-key 'normal (kbd "RET")
                              (lambda ()
                                ...
                                (let ((line (string-trim (thing-at-point 'line t))))
                                  ...
                                  (helios/buffer/spawn "main" #'helios/thread/content (assoc "main" helios/threads))
                                  (switch-to-buffer "*helios/main*")
                                  ;; search for line and jump to it
                                  (when (search-forward line nil t)
                                    (recenter 0)))))))

      ;; enable arrow key day navigation in recall threads
