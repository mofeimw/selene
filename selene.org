...
(defface helios/font/code
  '((t (:foreground "#f7768e" :height 140 :family "Fira Code")))
...
(defun helios/command/countdown (args)
  (if (string-match "\\(?:\\(.*?\\)\\s-*\\)?\\[\\([0-9]+\\([/.]\\)[0-9]+\\(?:\\3[0-9]+\\)?\\)\\]\\s-*$" args)
      (let* ((description (match-string 1 args))
             (date-string (match-string 2 args))
        ...
        (setq helios/countdown (cons (if (string-empty-p description) "" description) days-between)))
