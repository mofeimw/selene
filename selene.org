...
(defun helios/load-dashboard ()
  ...
  (with-current-buffer (helios/buffer)
    ...
    (let ((inhibit-read-only t))
      ;; erase buffer, insert random quote and increment line counter z by 2
      ;; increment z line counter accordingly onwards
      (erase-buffer)
      (insert "H E L I O S\n")
      (insert (format "\"%s\"\n" quote))
      (setq helios/dashboard/z (+ helios/dashboard/z 2))
      ...
      (when helios/countdown
        ...
        ;; increment counter
        (setq helios/dashboard/z (+ helios/dashboard/z 1)))

      ;; insert heading if there are todos
      ...
      (mapcar (lambda (pair)
                ;; show continuation if overflowing
                (if (= helios/dashboard/z 13)
                    (insert "...\n"))

                ;; insert word formatted to emphasize keybind in brackets
                ;; split where first occurance of letter is and piece together formatted
                (let* ((word (car pair))
                      (letter (cdr pair))
  ...
  ;; enable scrolling if enough number of lines, otherwise disable
  (if (< helios/dashboard/z 14)
      (progn
...
(defun helios/generate-thread-keys ()
  ...
  (let ((used-letters (make-hash-table))
    ...
    ;; set to reverse because push adds to front
    (setq helios/threadkeys (nreverse result))))
