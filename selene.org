...
(defun helios/utility/watchdog ()
  ...
  ;; make sure only one timer instance is ever running
  ...
  (unless helios/midnight
    ;; timers are triggered immediately if past the time set
    ;; so this will also initialize helios on startup
...
;; spawn helios watchdog during startup
(add-hook 'emacs-startup-hook #'helios/utility/watchdog)
;; switch to helios dashboard buffer created by midnight timer on launch
