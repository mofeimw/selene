...
(defun helios/dashboard/threads ()
  ...
  (let ((n 5)
    ...
    (catch 'break
      ...
      (mapcar (lambda (pair)
                ;; break to only show first n threads
                (when (>= count n)
                  (throw 'break nil))
                (let* ((word (car pair))
                       (letter (cdr pair))
                  ...
                  (if match
                    ...
                    ;; or stick letter at the end if it's random and not contained in the word
                    (progn
                      (insert word)
                      (insert (propertize (format "[%s]" letter) 'font-lock-face 'helios/font/emphasis))))
                  (insert "\n")
                  (setq count (+ count 1))))
              helios/threadkeys))))
...
(defun helios/dashboard/todo ()
  ...
  (let* ((featured)
         ...
         (featured-other)
         (n 6)
         ;; get all todos, sorted and classified
         (all (helios/todo/sort))
         ;; separate into classes
         (important (cdr (assoc "important" all)))
         (dated (cdr (assoc "dated" all)))
         (other (cdr (assoc "other" all))))
...
(defun helios/command/todo (args)
  ...
  (let* ((group-name)
    ...
    ;; assoc group from helios/todo
    ;; nil is the key for ungrouped todos
    (setq group (assoc group-name helios/todo))
    ...
    (if (string-match "\\[\\(.*\\)\\]" args)
          ...
          (cond
           ...
           ((string-match "[0-9]+\\([/.]\\)[0-9]+\\1[0-9]+" bracket-contents)
            ...
            (let ((date (helios/utility/parse-date bracket-contents)))
              (if date
                  (progn
                    ;; add to group or create new group and add to it
                    (if group
                        (setcdr group (cons (cons task date) (cdr group)))
                      (push (list group-name (cons task date)) helios/todo))
      ...
      (progn
        ...
        ;; '() or nil indicates no deadline
        (if group
            (setcdr group (cons (cons (string-trim args) '()) (cdr group)))
          (push (list group-name (cons (string-trim args) '())) helios/todo))
...
(defun helios/buffer/spawn (buffer-name content-function &optional args)
  ...
  (with-current-buffer (get-buffer-create (format "*helios/%s*" buffer-name))
    ...
    (let ((inhibit-read-only t))
      ...
      ;; mark todos as completed quicker
...
(defun helios/todo/view ()
  ...
  ;; insert ungrouped todos
  (let ((todos (assoc nil helios/todo)))
    (helios/todo/view/render-group (cdr todos)))

  ;; insert all other grouped todos
  (dolist (group helios/todo)
    ;; don't insert ungrouped todos again
    (when (car group)
      (insert (propertize (concat (car group) "\n") 'font-lock-face 'helios/font/h4))
      (helios/todo/view/render-group (cdr group)))))

(defun helios/todo/view/render-group (group)
  "render group of todos"
  ;; save completed todos for the end
  (let ((completed '()))
    ;; loop over todos
    (dolist (todo group)
      ;; set aside completed todos
      (if (equal (cdr todo) '("x"))
    ...
    ;; render completed todos greyed out
...
(defun helios/todo/sort ()
  ...
  (let* ((all-important '())
    ...
    (dolist (group helios/todo)
      ...
      (let ((important '())
        ...
        (dolist (todo (cdr group))
          ;; set aside important todos
          (if (string-match ".*!!.*" (car todo))
              (push todo important)
            ;; set aside dated todos
            (if (= (length (cdr todo)) 3)
                (push todo dated)
              ;; set aside completed todos
              (if (equal (cdr todo) '("x"))
                  (push todo done)
        ...
        (setf (cdr group) (append (reverse important) dated (reverse other) (reverse done)))

        ;; retain all but completed todos
    ...
    ;; return amalgamated assoc list of classes
    (list (append '("important") all-important)
