  ...
  (cond
   ((string-match "^#\\([a-zA-Z0-9_-]+\\)" line)
    (let* ((tag (match-string 1 line))
           (tag-thread (assoc tag *threads*)))
      (if tag-thread
          (setcdr tag-thread (cons line (cdr tag-thread)))
        (push (list tag line) *threads*)))))
  (unless (string= line "")
    (let ((main-thread (assoc "main" *threads*)))
      (if main-thread
          (setcdr main-thread (cons line (cdr main-thread)))
        (push (list "main" line) *threads*)))))

(defun helios/load-dashboard ()
  (with-current-buffer (helios/buffer)
    (let ((inhibit-read-only t))
      (erase-buffer)
      (insert (helios/center-vert-pad))
      (insert (helios/center-text "welcome to HELIOS"))
      (insert (helios/center-text "=================\n"))
      (insert (helios/center-text "threads"))
      (insert (helios/center-text "-------"))
      (dolist (thread *threads*)
        (insert (helios/center-text (concat "[" (downcase (substring (car thread) 0 1)) "] " (car thread)))))
      (goto-char (point-min))))
...
(defun helios/center-vert-pad ()
  (let* ((height (window-total-height)))
    (make-string (/ height 4) ?\n)))

(defun helios/center-text (text)
  (let* ((width (- (window-total-width) 16))
  ...
  (dolist (thread *threads*)
    (let* ((name (car thread))
      ...
      (evil-local-set-key 'normal (kbd key)
                          `(lambda ()
                             (interactive)
                             (helios/display-thread ',thread))))))

(defun helios/display-thread (thread)
  (print thread))
