...
(defvar helios/calendar/date (calendar-current-date))
(defvar helios/calendar/period 'month)
  ...
  (cond
   ...
   ((string-match "\\(.*?\\)\\s-*\\(?:\\[\\([0-9]+\\([/.]\\)[0-9]+\\(?:\\3[0-9]+\\)?\\)\\]\\)?\\s-*$" args)
    ...
    (let* ((task (match-string 1 args))
      ...
      ;; add to calendar if dated and uncompleted
      (when (not (or (string-empty-p date-string) (string= date-string "x")))
        (push (cons (helios/utility/parse-date date-string separator) task) helios/events))
...
(defun helios/command/schedule (args)
  ...
  (if (string-match " \\([0-9]+\\([/.]\\)[0-9]+\\(?:\\2[0-9]+\\)?\\).*" args)
      ;; pull out date, separator, event, and then parse date string into list
      (let* ((date-string (match-string 1 args))
             (separator (match-string 2 args))
...
(defun helios/calendar/svg/month (&optional date-arg)
  ...
  (let* ((cell-width 190)
    ...
    ;; resize cells to make all months fit on screen
    (if (> weeks-in-month 5)
    ...
    (let ((day 1))
      ...
      (dotimes (i (* 7 weeks-in-month))
        ...
        (when (and (>= i first-weekday) (<= day last-day))
          ;; calculate coordinates and format current date as list
          (let* ((x (+ 25 (* (mod i 7) cell-width)))
                 (y (+ 90 (* (/ i 7) cell-height)))
                 (date-list (list month day year))
                 ;; keep track of starting y-coordinate and also day color
                 (start-y y)
                 (day-color "#a9b1d6"))
            ...
            (dolist (event (reverse helios/events))
              ...
              (when (equal date-list (car event))
                ;; don't overflow cell - see how much space has been used since starting
                (when (< (- y start-y) 90)
                  ...
                  (let ((color "#a9b1d6"))
                    ...
                    (setq y (helios/calendar/svg/wrapped-text (cdr event)
                                                              23 x y 16 color)))
                  ;; pad events slightly
                  (setq y (+ 7 y)))))

            ;; change color of day if overflowed
            (if (> (- y start-y) 80)
...
(defun helios/calendar/svg/week (&optional date-arg)
  ...
  (let* ((cell-width 190)
    ...
    (let ((current-date week-start))
      ...
      (dotimes (i 7)
        ;; calculate coordinates and get date as list
        (let ((x (+ 30 (* i cell-width)))
              (y 90)
              (date-list current-date))
          ;; render month/day centered in box
          (svg-text svg
                    (format "%d/%d" (car date-list) (cadr date-list))
...
(defun helios/calendar/svg/wrapped-text (str limit x y font-size color)
  "render svg text with wrapped line breaks"
  ;; split words by space, init variables, and keep track of y-coordinate
  (let ((words (split-string str))
    ...
    ;; don't forget stragglers
    (if (not (string-empty-p line))
        (push line lines))

    (dolist (line (reverse lines))
      ;; render line and update y-coordinate
      (svg-text svg
                line
                :x x
                :y line-y
    ...
    ;; return y-coordinate so other events aren't rendered right on top
    line-y))
