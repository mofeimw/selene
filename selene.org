...
(defun helios/dashboard ()
  ...
  ;; bind recall view to both @ and 3
...
(defun helios/calendar/svg ()
  "render calendar in svg"
  (let* ((cell-width 190)
         (cell-height 140)
         (svg-width (+ 40 (* 7 cell-width)))
         (svg-height (+ 150 (* 6 cell-height)))
         ;; create svg with calculated dimensions plus padding
    ...
    (let ((day 1))
      ...
      (dotimes (i (* 7 weeks-in-month))
        ...
        (when (and (>= i first-weekday) (<= day last-day))
          ...
          (let ((x (+ 30 (* (mod i 7) cell-width)))
                (y (+ 145 (* (/ i 7) cell-height)))
                (date-list (list month day year)))
            ;; render number in corner
            (svg-text svg
