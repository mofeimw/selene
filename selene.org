    ...
    (dolist (todo helios/todo)
      ...
      (if (string-match "\\([0-9]+\\([/.]\\)[0-9]+\\(?:\\2[0-9]+\\)?\\)" (cdr todo))
          ...
          (let* ((separator (match-string 2 (cdr todo)))
            ...
            ;; upcoming items are in the next two weeks
            (if (< days-between 15)
                (progn
...
(defun helios/command/include (args)
  ...
  (when (file-exists-p (string-trim args))
...
(defun helios/spawn-recall-thread ()
  ...
  (interactive)
  ;; get the line the cursor is on and strip of newline
  ...
  (let ((day-start-flag nil))
    (dolist (entry (assoc "main" helios/threads))
      (when day-start-flag
        ...
        ;; simply render single string entries
        (when (stringp entry)
          (insert entry)
          (insert "\n\n")))
