      ...
      (let* ((day-start (caaar sorted-blocks))
        ...
        (dolist (block sorted-blocks)
          ...
          (let* ((times (car block))
            ...
            ;; draw caption text
            (svg-text svg
                      ;; format neatly - extract minutes substring of military time to retain leading 0s
                      (format "[%d:%s-%d:%s] %s"
                              (car start-time) (substring start-time-military-string
                                                          (- (length start-time-military-string) 2)
                                                          (length start-time-military-string))
                              (car end-time) (substring end-time-military-string
                                                        (- (length end-time-military-string) 2)
                                                        (length end-time-military-string))
                              event)
                      ;; center text horizontally
                      :x (+ 80 cell-width (/ timeblock-width 2))
